...
mybinpost <- function(n, x, theta, alpha) {
  ...
  # CALCULATIONS -------------------------------------------
    ...
    ## Consolidate into a matrix with column names
    bayesMatrix <- matrix(data = c(theta, prior, likelihood, h, posterior), 
                        nr = n, nc = 5, byrow = FALSE)
    colnames(bayesMatrix) <- c("theta", "prior", "likelihood", "h", "post")
...
## `a.` `mybinpost(n=20,x=12,theta = seq(0,1,length=50), alpha=0.05)`
```{r 5a, echo=TRUE, message=FALSE}
mybinpost(n=20,x=12,theta = seq(0,1,length=50), alpha=0.05)
```

## `b.` `mybinpost(n=50, x=25, theta=seq(0,1,length=30), alpha=0.1)`
```{r 5b, echo=TRUE, message=FALSE}
mybinpost(n=20, x=10, theta=seq(0,1,length=20), alpha=0.05)
