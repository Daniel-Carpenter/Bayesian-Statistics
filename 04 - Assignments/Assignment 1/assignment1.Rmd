...
* Below *Derived from Statistics for Engineering and the Sciences, Sixth Edition*
* William M. Mendenhall, Terry L. Sincich
* Taylor & Francis, Jan 15, 2016 - Mathematics - 500 pages
...
M_x(t) = E(e^{Xt}) 
= \sum_Xe^{Xt}p(x) 
= \sum_Xe^{xt}\begin{pmatrix}n \\ x \end{pmatrix}p^xq^{n-x}
= \sum_X\begin{pmatrix}n \\ x \end{pmatrix}(pe^t)^xq^{n-x}
$$

* Binomial theorem defined as:  

$$
(a + b)^n = \sum_X\begin{pmatrix}n \\ x \end{pmatrix}a^xb^{n-x}
$$

* If $a = pe^t$ and $b = q$, then $m(t) = (pe^t + q)^n$
  
## `b.`

### `i.`
Since $\left.\mu_{k}^{\prime}=\frac{d^{k} m(t)}{d t^{k}}\right]_{t=0}$, then 
$$
\begin{aligned}
\mu_{1}^{\prime} &\left.\left.=\mu=\frac{d m(t)}{d t}\right]_{t=0}=n\left(p e^{t}+q\right)^{n-1}\left(p e^{t}\right)\right]_{t=0} \\
&=n\left(p e^{0}+q\right)^{n-1}\left(p e^{0}\right)
\end{aligned}
$$

Since $e^{0}=1$, then $np = E(x)$, which is defined below as $\mu_{1}^{\prime}$
$$
\mu_{1}^{\prime}=\mu=n(p+q)^{n-1} p=n(1)^{n-1} p=n p
$$

Now get $\mu_{2}^{\prime}$
$$
\begin{aligned}
\mu_{2}^{\prime} &\left.\left.=\frac{d^{2} m(t)}{d t^{2}}\right]_{t=0}=n p \frac{d}{d t}\left[e^{t}\left(p e^{t}+q\right)^{n-1}\right]\right]_{t=0} \\
&\left.=n p\left[e^{t}(n-1)\left(p e^{t}+q\right)^{n-2} p e^{t}+\left(p e^{t}+q\right)^{n-1} e^{t}\right]\right]_{t=0} \\
&=n p[(1)(n-1)(1) p+(1)(1)]=n p[(n-1) p+1] \\
&=n p(n p-p+1)=n p(n p+q)=n^{2} p^{2}+n p q
\end{aligned}
$$


### `ii.`
From `i`, we can get the variance
$$
\sigma^{2}=E\left(X^{2}\right)-\mu^{2}=\mu_{2}^{\prime}-\left(\mu_{1}^{\prime}\right)^{2}
$$


### `iii.`
The above calculation in `ii.` gives us $\mu_{2}^{\prime}$ and $\mu_{1}^{\prime}=\mu$, so $V(X) = npq$
$$
\sigma^{2}=n^{2} p^{2}+n p q-(n p)^{2}=n p q
$$
