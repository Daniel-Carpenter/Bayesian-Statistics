...
# Run the fun and store
codaSamplesLogitCenter <- runJagsFunction(modelStringLogitCenter, parameters, nIter=5000, # Total iterations
...
valuesOf = 'p' # the value of interest
...
actualData <- data.frame(y = dataList$y, 
                         # n = dataList$n,#intentially do not include n
                         x = dataList$x)
...
df <- df.codaSummary %>%
  ...
  # pivot_longer(cols      = !c(statName, x),
  #              names_to  = 'estName',
  #              values_to = 'estValue')
...
df %>%
  ggplot(aes(x=x)) + 
  geom_line(aes(y=lowerTail), linetype='dashed') + 
  geom_line(aes(y=mean)) + 
  geom_line(aes(y=upperTail), linetype = 'dashed')
