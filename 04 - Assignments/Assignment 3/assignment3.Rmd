...
# I would choose 50 but it took an eroneous time to run. This case only uses 1 chain.
# Note the implication is high uncertainty in estimates since base on only 1 simulated data sets.
nSimulatedDataSets = 1 # min(500,NROW(mcmcMat)) 
```
...
# Note the -1 at end. 
# Again, only using one simulation because it my computer cannot handle the simulations
# Note the implication is high uncertainty in estimates since base on only 1 simulated data sets.
...
nSimulatedDataSets = 1 # still 1 since takes too long to run for an exercise
```
