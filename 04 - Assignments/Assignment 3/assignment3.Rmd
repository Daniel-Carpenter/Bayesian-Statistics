...
# 2-values in vector proposal. First value is success, second is failure
myprop <- function(n, p, proposal=c(0.3, 0.6)){ 
  out <- sample(proposal, size = n,
...
## `b` Call `myprop()`
```{r mypropCall}
# Given proposal c(0.3, 0.6) by default...
...
createBarPlot <- function(myprop, n, p) {
  ...
  # Create the barplot
  barplot(table(myprop) / n, 
          main = paste0('Proposal Distribution from myprop(n,p) | p = ',p,'\n Daniel Carpenter'),
...
# Call the bar plot function and tell it what n and p are
# Note Given proposal c(0.3, 0.6) by default...
createBarPlot(myprop(n, p), n, p)
```
