...
# Prior to R 4.0, read.csv() delivered strings a factors, but not with R 4.0+.
# Therefore convert strings to factors, for the purpose of these examples.
# First, notice the strings are character vectors, not factors:
str( HGNdf )
# Now convert to factors:
HGNdf$Hair = factor( HGNdf$Hair )
HGNdf$Gender = factor( HGNdf$Gender )
HGNdf$Name = factor( HGNdf$Name )
# Notice they are now factors:
str( HGNdf )
